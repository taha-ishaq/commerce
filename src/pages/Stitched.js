...
const Stitched = ({ addToCart }) => {
    ...
    useEffect(() => {
      const fetchProducts = async () => {
        try {
          const response = await axios.get('http://localhost:5000/products?tag=stitched');
  ...
  return (
    ...
    <Box sx={{ marginTop: '100px', paddingTop: '20px' }}>
      {products.length === 0 ? (
        <Typography variant="h6" align="center">No Stitched products available</Typography>
      ) : (
        <>
